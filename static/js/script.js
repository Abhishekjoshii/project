...
const getWeatherData = async (latitude, longitude) => {
    let weatherApiParams = new URLSearchParams({
        latitude: latitude,
        longitude: longitude,
        daily: "precipitation_sum", // Get daily precipitation sum
        hourly: "temperature_2m,relative_humidity_2m", // Still get hourly temperature and humidity
        timezone: "auto" // Automatically adjust to user's timezone
    });
    ...
    try {
        ...
        const dailyPrecipitation = weatherData.daily.precipitation_sum[0]; // Get today's total precipitation
        const temperature = weatherData.hourly.temperature_2m[0]; // Current temperature
        const humidity = weatherData.hourly.relative_humidity_2m[0]; // Current humidity

        weatherInfo.textContent = `
            Current temperature: ${temperature}°C,
            Humidity: ${humidity}%,
            Precipitation: ${dailyPrecipitation}mm (daily)
        `;
        
        // Recommend crops based on weather data
        const recommendations = recommendCrops(temperature, humidity, dailyPrecipitation);
